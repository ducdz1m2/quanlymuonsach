    ...
    <div class="d-flex flex-column bg-dark text-white sidebar" :class="{ 'collapsed': isCollapsed }">
        ...
        <ul class="list-unstyled mt-3">
            <li v-for="(item, index) in filteredMenuItems" :key="index"
                class="px-3 py-2 d-flex align-items-center menu-item" :class="{ 'active': activeIndex === index }"
    ...
    <div v-if="showHelp" class="help-carousel-overlay">
        <div class="help-carousel">
            ...
            <div class="help-buttons">
                <button class="btn btn-primary" @click="prevStep" :disabled="currentStep === 0">Previous</button>
                <button class="btn btn-primary" @click="nextStep"
                    :disabled="currentStep === helpSteps.length - 1">Next</button>
                <button class="btn btn-secondary" @click="closeHelp">Close</button>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        filteredMenuItems() {
            const helpItem = this.menuItems.find(item => item.text === 'Trợ giúp');
            let items = [];
    ...
    methods: {
        ...
        updateActiveIndexByRoute(path) {
            const index = this.filteredMenuItems.findIndex(item => item.route === path);
            if (index !== -1) this.activeIndex = index;
        },
        nextStep() {
            if (this.currentStep < this.helpSteps.length - 1) this.currentStep++;
        },
        prevStep() {
